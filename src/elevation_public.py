...
teste = pd.read_csv('indices_public.csv', index_col=0)
...
data17_sp = data17[data17['MUNI_D'] == 36] # Only SP
...
data17_carros = data17_sp[data17_sp['MODOPRIN'].isin(['metro', 'trem', 'onibus'])] 
...
data_menor = data_menor.head(11350)
def calculate_distance_openservice(row):  
    if row.name not in dict_routes.keys(): 
        try:
            ...
            print(coords)
            client = openrouteservice.Client(key='5b3ce3597851110001cf6248758acbaf705a464388f196e85dab1a24') # Specify your personal API key
...
teste.to_csv('indices_public.csv')
